...
Claude Code의 hook 시스템은 settings.json에 정의된 외부 명령을 특정 이벤트마다 실행한다. plz-save-token는 두 종류의 hook을 제공:
...
**핵심 원칙:** stderr 경고만, exit 0 (블로킹 ❌). 사용자가 결정. plz-save-token는 가이드 역할.
...
| 패턴 | 검출 keyword | 출력 |
|------|-------------|------|
| 결정적 keyword + opus | `regex`, `verbatim`, `BibTeX`, `CSV→JSON`, ... | `[plz-save-token PRE-FLIGHT] 결정적 keyword 감지: '<reason>' (matrix #N). Python 분기 검토.` |
| opus + 작은 작업 | `opus` + `grep`/`탐색`/`boilerplate` | `[plz-save-token PRE-FLIGHT] opus + 탐색/짧은 작업 — Haiku 가능 [task_to_model_matrix #2·22]` |
| 5+ agent fan-out | description에 "5인", "8인", "10명" | `[plz-save-token PRE-FLIGHT] N+ agent fan-out — 단일 세션 가능성 점검? [A003 7× cost]` |
| model_selector 추천 | 자동 호출 결과 | `[plz-save-token model_selector] 추천 <model>, vs Opus 절감 N%` |
...
| 패턴 | 검출 정규식 | 출력 |
|------|------------|------|
| All-opus | `모두\s*opus`, `전부\s*opus` | `[plz-save-token OVERUSE] 모든 agent을 opus로 — anti-pattern [C1.1, 사용자 99.0% 안 됨]` |
...
python3 /path/to/plz-save-token/scripts/hook_check.py self-test

# 실제 payload 흐름 시뮬레이션 (stdin)
echo '{"tool_name":"Task","tool_input":{"description":"BibTeX 생성"}}' | \
  python3 /path/to/plz-save-token/scripts/hook_check.py pretooluse

# UserPromptSubmit 시뮬레이션
echo '{"prompt":"5인 팀 만들어서 opus로 regex 작업"}' | \
  python3 /path/to/plz-save-token/scripts/hook_check.py userprompt
```
...
```bash
# Claude Code 세션 환경변수
export PLZ_SAVE_TOKEN_HOOK_DISABLE=1
```

(이 기능을 hook_check.py에 추가하려면 `os.environ.get("PLZ_SAVE_TOKEN_HOOK_DISABLE")` 체크 후 즉시 return.)
...
# 2. 스크립트 실행 권한
chmod +x /path/to/plz-save-token/scripts/hook_check.py

# 3. 의존 모듈 확인
python3 -c "from /path/to/plz-save-token/scripts/model_selector import recommend"

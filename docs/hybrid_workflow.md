# Hybrid workflow: harness-diagnostic + plz-save-token

> Two complementary tools for multi-agent Claude Code harnesses. Diagnose structurally with harness-diagnostic (HD), then act on cost and runtime gaps with plz-save-token.

Same authoring lineage (`paper-maker` observations); complementary detection surface. HD catches gaps invisible at file level; plz-save-token catches cost traps invisible at the structural-gap level.
...
| [plz-save-token](https://github.com/epoko77-ai/plz-save-token) | "Where is my harness burning tokens and what do I do about it?" | 9 rules × cost-impact audit, model decision tree, cost estimator, runtime hook |
...
# Step 2 — cost-impact audit (plz-save-token)
python3 plz-save-token/scripts/audit.py /path/to/your/harness

# Step 3 — read plz-save-token fix_plan output, prioritize by ROI
python3 plz-save-token/scripts/model_selector.py --task "<agent role description>"
...
| Pattern | HD rule | plz-save-token rule | Coverage |
|---------|---------|----------------|----------|
...
| cache_control breakpoints absent | — | R5 (C5.1) | plz-save-token only |
| Multi-agent 7x multiplier | — | R3 (C3.1) | plz-save-token only |
| Full file read vs offset/limit | — | R6 (C7.1) | plz-save-token only |
| SKILL.md / CLAUDE.md body bloat | — | R4 (C4.1), R9 (C4.5) | plz-save-token only |
...
The 4 overlapping patterns (HD-003, HD-010, HD-011, HD-020) are both reliability bugs and cost bugs — HD names the structural absence, plz-save-token names the cost class. Expect overlapping FAIL signals on those four when running both tools.

---

## Adoption story

When building plz-save-token's taxonomy, patterns from harness-diagnostic were evaluated against the N>=2 external-corroboration rule: a pattern enters the plz-save-token catalog only if two or more independent external sources confirm it.

Result:

| HD gap | Adopted into plz-save-token? | Reason |
|--------|------------------------|--------|
...
HD users running plz-save-token on top should expect:
...
- **Complementary signals** on HD-001/006/009/018 — these have no plz-save-token counterpart.
...
HD-001 FAIL only (no plz-save-token counterpart)
